...
abstract class dmSearchIndex extends dmSearchIndexCommon
{
	protected
	$location,
	$index,
...
//		return new Zend_Search_Lucene_Search_Query_Fuzzy($term, 0.4);
	...
	public function populate()
	{
		...
		foreach ($pages as $page)
		{
			...
			$this->index->addDocument(new mySearchPageDocument($page));
		}
	...
	{
		dmFilesystem::get()->deleteDirContent($this->location);
		$this->index = Zend_Search_Lucene::create($this->location);
	}
	...
	protected function getPagesQuery()
	{
		...
		->where('translation.is_approved = ? AND p.is_secure = ? AND ( p.module != ? OR p.action != ?)', array(true, false, 'main', 'error404'));
	}
