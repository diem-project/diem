...
echo _open('div#dm_tool_bar.dm.clearfix.'.sfConfig::get('dm_toolBar_flavour', 'blue'));
  ...
  echo _link('app:admin')->textTitle(__('Administration'))->set('.widget16.s16block.s16_home');
  
  if ($sf_user->can('clear_cache'))
  {
    echo _link('+/dmCore/refresh')->textTitle(__('Clear cache'))->set('.dm_refresh_link.widget16.s16block.s16_clear');
  }
  
  if($sf_user->can('code_editor'))
  {
    echo _link('+/dmCodeEditor/launch')->textTitle(__('Code Editor'))->set('.code_editor.widget16.s16block.s16_code_editor');
  }
  ...
  {
    echo _link('+/dmPage/edit')->set('a.page_edit_form.widget24.s24block.s24_page_edit')->textTitle(__('Edit page'));
  }
  
  if ($sf_user->can('page_add'))
  {
    echo _link('+/dmPage/new')->set('a.page_add_form.widget24.s24block.s24_page_add')->textTitle(__('Add new page'));
  }
