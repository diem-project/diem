...
if (!empty($mldProjectImage))
{
  echo _tag('div.dm_box.big.diagram',
    _tag('div.title',
      _tag('h2', 'Project Database'._link($mldProjectImage)->text(__('Download'))->target('blank'))
    ).
...
if (!empty($mldCoreImage))
{
  echo _tag('div.dm_box.big.diagram',
    _tag('div.title',
      _tag('h2', 'Diem Core Database'._link($mldCoreImage)->text(__('Download'))->target('blank'))
    ).
    _tag('div.dm_box_inner',
...
foreach($dicImages as $appName => $image)
{
  ...
  echo _tag('div.dm_box.big.diagram',
    _tag('div.title',
    _tag('h2', dmString::camelize($appName).' : Dependency Injection Container'._link($image)->text(__('Download'))->target('blank'))).
    _tag('div.dm_box_inner',
      ($withDispatcherLinks
